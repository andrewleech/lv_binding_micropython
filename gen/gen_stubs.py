...
def generate_class_stub(class_name, class_info, doc_index=None,
                         parent=None, skip_members=None):
    """Generate a Python stub for a class.

    Args:
        class_name: Widget class name without the lv_ prefix.
        class_info: Metadata entry for the class (objects[name]).
        doc_index: Map of C function name to extracted docstring.
        parent: Base class name to inherit from, or None for no inheritance.
            When set, the emitted class is `class <name>(<parent>):` and
            anything in skip_members is omitted from this class's body.
        skip_members: Set of member names already provided by the parent
            class. Skipped here so methods aren't duplicated across the
            hierarchy.
    """
    header = f"class {class_name}({parent}):" if parent else f"class {class_name}:"
    lines = [header]
    ...
    # Generate methods. The metadata wraps members one level deep:
    # objects[name] = {"members": {method_name: {...}, ...}}
    members = class_info.get("members", class_info)
    ...
    # If the class inherits everything from its parent, give Python a body.
    if parent and body_lines == 0:
        lines.append("    pass")
...
def generate_main_stub(module_name, metadata, doc_index=None):
    ...
    # Add module-level functions. Metadata strips the lv_ prefix off function
    # names, but the source-walking doc index keeps the full C identifier, so
    # look both up.
    functions = metadata.get("functions", {})
    for func_name, func_info in functions.items():
        doc_info = None
        if doc_index:
            doc_info = (find_function_docs_in_sources(f"lv_{func_name}", doc_index)
                        or find_function_docs_in_sources(func_name, doc_index))

        lines.append(generate_function_stub(func_name, func_info, doc_info, c_func_name=f"lv_{func_name}"))
    
    # Add classes. LVGL widgets all inherit from obj at the C level — emit obj
    # first with its full method set, then each widget as `class <name>(obj):`
    # with inherited methods skipped to keep the stub compact.
    objects = metadata.get("objects", {})
    obj_info = objects.get("obj", {})
    obj_members = set((obj_info.get("members") or {}).keys())

    if "obj" in objects:
        lines.append(generate_class_stub("obj", obj_info, doc_index))

    for obj_name, info in objects.items():
        if obj_name == "obj":
            continue
        lines.append(generate_class_stub(obj_name, info, doc_index,
                                          parent="obj", skip_members=obj_members))
...
def validate_documentation_coverage(doc_index, metadata):
    ...
    for func_name in metadata["functions"]:
        if (find_function_docs_in_sources(f"lv_{func_name}", doc_index)
                or find_function_docs_in_sources(func_name, doc_index)):
